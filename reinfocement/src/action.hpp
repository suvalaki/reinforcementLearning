...
template <StateType STATE_T, spec::CompositeArraySpecType T> struct Action : T::DataType {
  ...
  virtual std::size_t hash() const { return static_cast<const typename T::DataType &>(*this).hash(); }
...
concept ActionType = std::is_base_of_v<Action<typename ACTION_T::StateType, typename ACTION_T::SpecType>, ACTION_T>;
