...
template <typename T>
concept AnyArraySpecType =
    isBoundedArraySpec<T> || isCategoricalArraySpec<T>;

template <typename... T>
concept AllElementsAnyArraySpecType = (AnyArraySpecType<T> && ...);

template <AllElementsAnyArraySpecType... T>
// requires AllElementsAnyArraySpecType<T...>
...
// isinstance of CompositeArraySpec
//  only if check all tuple elements are AnyArraySpecType
template <typename T> 
concept CompositeArraySpecType = hasTupleType<T> && 
