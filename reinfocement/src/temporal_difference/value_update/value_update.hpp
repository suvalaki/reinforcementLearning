...
auto temporal_differenc_error(const auto &valueAtStateT,
                              const auto &valueAtStateT1,
                              const auto &reward,
                              const auto &discountRate) {
...
template <typename CRTP, policy::objectives::isFiniteStateValueFunction VALUE_FUNCTION_T> struct TDValueUpdaterBase {
